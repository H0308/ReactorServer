...
// 从属线程管理
rs_loop_thread_pool::LoopThreadPool::ptr loops;
...
void handleAccept(int fd)
{
    ...
    rs_connection::Connection::ptr client = std::make_shared<rs_connection::Connection>(loops->getNextLoop(), id, fd);
...
void testBasicServerOperations()
{
    ...
    loops = std::make_shared<rs_loop_thread_pool::LoopThreadPool>(server_loop.get());
    loops->setThreadNum(2);
    loops->createLoopThread();
